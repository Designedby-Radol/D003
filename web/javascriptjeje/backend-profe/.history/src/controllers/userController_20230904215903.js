...
async function getFollowersByUsername(request, response) {
  try {
    const { username } = request.params;
    const user = await UserModel.findOne({ username }, { followers: 1 });
    return response.status(200).json({ followers: user.followers });
  } catch (error) {
    return response.status(400).json({ msg: "Error", error });
  }
}

async function getPossibleFollows(request, response) {
  try {
    let { _id } = request.params;
    const user_follows = await UserModel.findById(_id, { follows: 1, _id: 0 });
    console.log(follows);
    const data = await UserModel.find(
      // transformar todos los id´s en Object(ID)
      { "_id": { $nin: user_follows } },
      { username: 1, name: 1, lastname: 1 }
    );
    response.status(200).json({ data });
  } catch (error) {
    return response.status(400).json({ msg: "Error", error });
...
async function deleteFollower(request, response) {
  try {
    ...
    await UserModel.findByIdAndUpdate(id_no_sigue_a, {
      follows: no_sigue_a.follows,
    });
    ...
    await UserModel.findByIdAndUpdate(id_a_no_seguir, {
      followers: no_seguir_a.followers,
    });
    return response
      .status(400)
      .json({
        msg: "Se ha dejado de seguir un usuario",
        id_a_no_seguir,
        id_no_sigue_a,
      });
  } catch (error) {
